 ...
 */
export function expressionWithFirstItem(
  syntaxTree: Tree,
  expression: string
): string {
...
export const isAllowedInDotNotation = (str: string) => {
  const DOT_NOTATION_BANNED_CHARS =
    /^(\d)|[\\ `!@#$%^&*()+\-=[\]{};':"\\|,.<>?~]/g;
...
export function receivesNoBinaryData(contextNodeName?: string) {
  try {
    return (
      resolveAutocompleteExpression("={{ $binary }}", contextNodeName)?.data ===
      undefined
    );
  } catch {
    return true;
...
export function resolveAutocompleteExpression(
  ...
  const nodeData = getNodeDataStore();

  // targetItem: 提供表达式补全的目标 item，用于 $json[] 之类的路径预览
  // inputNodeName: 需要传入 resolveParameter 的输入节点名
  const inputData = {
    targetItem: nodeData.expressionTargetItem ?? undefined,
    inputNodeName: nodeData.inputNodeName,
  };
...
export const hasActiveNode = (
  ...
  (targetNodeParameterContext !== undefined &&
    getWorkflowsStore().getNodeByName(targetNodeParameterContext.nodeName) !==
      null) ||
  getNodeDataStore().activeNode?.name !== undefined;

export const isSplitInBatchesAbsent = () => {
  const nodes = Object.values(!getWorkflowsStore().workflow.nodes)
  
  return nodes.length > 0 && nodes.some(
    (node) => node.type === SPLIT_IN_BATCHES_NODE_TYPE
  )
}

export function autocompletableNodeNames() {
  const activeNode = getNodeDataStore().activeNode;
  if (!activeNode) return [];

  const activeNodeName = activeNode.name;

  const workflow = getWorkflowHelpers().getCurrentWorkflow();
  const nonMainChildren = workflow.getChildNodes(
    activeNodeName,
    "ALL_NON_MAIN"
  );
 ...
 */
export const stripExcessParens =
  (context: CompletionContext) => (option: Completion) => {
    const followedByParens =
    ...
    if (commonClosingChars > 0) {
      option.apply = (
        ...
        const tx: TransactionSpec = {
          ...insertCompletionText(
            view.state,
            option.label.slice(0, -commonClosingChars),
            from,
            to
          ),
...
export const applyCompletion =
  ...
  (
    ...
    const label = insertDefaultArgs(
      transformLabel(completion.label),
      defaultArgs
    );
    const tx: TransactionSpec = {
...
export const hasRequiredArgs = (doc?: DocMetadata): boolean => {
  if (!doc) return false;
  const requiredArgs =
    doc?.args?.filter((arg) => !arg.name.endsWith("?") && !arg.optional) ?? [];
  return requiredArgs.length > 0;
};

export const sortCompletionsAlpha = (
