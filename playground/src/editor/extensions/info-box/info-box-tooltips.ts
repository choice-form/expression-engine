...
import type { createInfoBoxRenderer } from "./info-box-render";
...
const createStateReader =
  (state: EditorState) => (node?: SyntaxNode | null) => {
...
function getJsNodeAtPosition(state: EditorState, pos: number, anchor?: number) {
  ...
  const isSelectionWithinNode = (n: SyntaxNode) => {
    return (
      jsPos >= n.from && jsPos <= n.to && jsAnchor >= n.from && jsAnchor <= n.to
    );
  };
...
function getInfoBoxTooltip(state: EditorState): Tooltip | null {
  ...
  const { node, pos, isSelectionWithinNode, getGlobalPosition, readNode } =
    jsNodeResult;
...
export const hoverTooltipSource = (view: EditorView, pos: number) => {
  ...
  const tooltipForNode = (subject: SyntaxNode) => {
    const completion = getCompletion(
      ...
      getGlobalPosition(subject.to - 1),
      (c) =>
        c.label === readNode(subject) || c.label === readNode(subject) + "()"
    );

    const newHoverTooltip = completionToTooltip(
      completion,
      getGlobalPosition(subject.from),
      {
