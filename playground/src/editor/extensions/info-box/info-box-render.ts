...
const renderDescription = ({
  ...
  if (docUrl) {
    ...
    //   i18n.autocompleteUIValues.docLinkLabel ??
    //   i18n.baseText("generic.learnMore");

    descriptionLink.innerText = "generic.learnMore";
...
const renderArg = (arg: DocMetadataArgument, highlight: boolean) => {
  ...
  if (!!arg.optional || arg.name.endsWith("?")) {
    tags.push("codeNodeEditor.optional");
  }
  ...
  if (arg.description) {
    ...
    if (
      arg.default &&
      arg.optional &&
      !arg.description.toLowerCase().includes("default")
    ) {
      const separator = arg.description.endsWith(".") ? " " : ". ";

      const baseText = "codeNodeEditor.defaultsTo" + arg.default;
      arg.description += separator + baseText;
    }
...
const renderArgList = (
  ...
  args.forEach((arg, index) => {
    argsList.appendChild(
      renderArg(arg, shouldHighlightArgument(arg, index, highlightArgIndex))
    );
  });
...
const renderArgs = (
  ...
  argsTitle.textContent = "codeNodeEditor.parameters";
...
const renderExamples = (examples: DocMetadataExample[]) => {
  ...
  examplesTitle.textContent = "codeNodeEditor.examples";
