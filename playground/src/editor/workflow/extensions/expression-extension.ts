...
export const EXPRESSION_EXTENSIONS: ExtensionMap[] = [
  arrayExtensions,
  dateExtensions,
  numberExtensions,
  objectExtensions,
...
const isExpressionExtension = (str: string) =>
  EXPRESSION_EXTENSION_METHODS.some((m) => m === str);
  ...
  try {
    ...
    visit(ast, {
      visitChainExpression(path) {
        ...
        const cancelIdentifier = types.builders.identifier(
          `chainCancelToken${chainNumber}`
        );
        const valueIdentifier = types.builders.identifier(
          `chainValue${chainNumber}`
        );
        ...
        const buildCancelCheckWrapper = (
          node: ExpressionKind
        ): ExpressionKind => {
          const result = types.builders.conditionalExpression(
        ...
        const buildValueAssignWrapper = (
          node: ExpressionKind
        ): ExpressionKind => {
          return types.builders.assignmentExpression(
            "=",
        ...
        const buildCancelAssignWrapper = (
          node: ExpressionKind
        ): ExpressionKind => {
          return types.builders.assignmentExpression(
            "=",
        ...
        const updatePatch = (toPatch: ExpressionKind, node: ExpressionKind) => {
          if (
            toPatch.type === "MemberExpression" ||
            toPatch.type === "OptionalMemberExpression"
          ) {
            toPatch.object = node;
          } else if (
              ...
              if (wrapNextTopInOptionalExtend) {
                ...
                if (
                  ...
                  patchTop = types.builders.callExpression(
                    ...
                    [
                      patchTop.object,
                      types.builders.stringLiteral(patchTop.property.name),
                    ]
                  );
                }
              }
        ...
        if (
          ...
          path.replace(
            types.builders.callExpression(
              types.builders.identifier(EXPRESSION_EXTENDER),
              [
...
function findExtendedFunction(
  input: unknown,
  functionName: string
): FoundFunction | undefined {
...
export function extend(input: unknown, functionName: string, args: unknown[]) {
  ...
  if (!foundFunction) {
    ...
    const haveFunction = EXPRESSION_EXTENSIONS.filter(
      (v) => functionName in v.functions
    );
...
export function extendSyntax(
  bracketedExpression: string,
  forceExtend = false
): string {
  ...
  if (
    (!codeChunks.some(hasExpressionExtension) ||
      hasNativeMethod(bracketedExpression)) &&
    !forceExtend
  ) {
