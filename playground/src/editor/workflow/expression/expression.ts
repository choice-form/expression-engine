...
import {
  evaluateExpression,
  setErrorHandler,
} from "./expression-evaluator-proxy";
import { sanitizer, sanitizerName } from "./expression-sandboxing";
import { extend, extendedFunctions, extendOptional } from "../extensions";
...
export class Expression {
  ...
  constructor(workflow: Workflow) {
    ...
    setErrorHandler((error: Error) => {
      ...
      if (isTypeError(error)) throw new ApplicationError("invalid type");
    });
  }
    ...
    if (value instanceof Date) {
      ...
      result =
        DateTime.fromJSDate(value, {
          zone:
            this.workflow.settings?.timezone ??
            getGlobalState().defaultTimezone,
        }).toISO() ?? "";
    ...
    if (typeof returnValue === "function") {
      ...
      if (returnValue.name === "DateTime")
        throw new ApplicationError(
          "this is a DateTime, please access its methods"
        );
