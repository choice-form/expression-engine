...
export const tryToParseNumber = (value: unknown): number => {
  ...
  if (!isValidNumber) {
    throw new ApplicationError("Failed to parse value to number", {
      extra: { value },
    });
  }
  return Number(value);
...
export const tryToParseAlphanumericString = (value: unknown): string => {
  ...
  if (!regex.test(parsed)) {
    throw new ApplicationError("Value is not a valid alphanumeric string", {
      extra: { value },
    });
  }
  return parsed;
...
export const tryToParseTime = (value: unknown): string => {
  ...
  if (!isTimeInput) {
    throw new ApplicationError("Value is not a valid time", {
      extra: { value },
    });
  }
  return String(value);
...
export const tryToParseArray = (value: unknown): unknown[] => {
  try {
    ...
    if (!Array.isArray(parsed)) {
      throw new ApplicationError("Value is not a valid array", {
        extra: { value },
      });
    }
    return parsed;
...
export const tryToParseObject = (value: unknown): object => {
  ...
  try {
    ...
    if (typeof o !== "object" || Array.isArray(o)) {
      throw new ApplicationError("Value is not a valid object", {
        extra: { value },
      });
    }
    return o;
...
export const tryToParseJsonToFormFields = (
  value: unknown
): FormFieldsParameter => {
  const fields: FormFieldsParameter = [];

  try {
    const rawFields = jsonParse<Array<{ [key: string]: unknown }>>(
      value as string,
      {
        acceptJSObject: true,
      }
    ...
    for (const [index, field] of rawFields.entries()) {
      for (const key of Object.keys(field)) {
        ...
        if (
          key === "fieldType" &&
          !ALLOWED_FIELD_TYPES.includes(field[key] as string)
        ) {
          throw new ApplicationError(
...
export const tryToParseUrl = (value: unknown): string => {
  ...
  if (!urlPattern.test(String(value))) {
    throw new ApplicationError(
      `The value "${String(value)}" is not a valid url.`,
      {
        extra: { value },
      }
    );
...
export function validateFieldType(
  ...
  switch (type.toLowerCase()) {
    ...
    case "options": {
      const validOptions = valueOptions
        .map((option) => option.value)
        .join(", ");
      const isValidOption = valueOptions.some(
