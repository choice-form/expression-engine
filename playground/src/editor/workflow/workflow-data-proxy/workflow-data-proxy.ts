...
} from "./workflow-data-proxy-env-provider";
...
import { SCRIPTING_NODE_TYPES } from "@/editor/extensions/constants";
...
type PairedItemMethod =
  (typeof PAIRED_ITEM_METHOD)[keyof typeof PAIRED_ITEM_METHOD];
...
export class WorkflowDataProxy {
  ...
  constructor(
    ...
    this.timezone =
      workflow.settings?.timezone ?? getGlobalState().defaultTimezone;
  ...
  private nodeContextGetter(nodeName: string) {
    ...
    if (
      !that.runExecutionData?.executionData &&
      that.connectionInputData.length > 0
    ) {
    ...
    if (
      !that.runExecutionData?.executionData &&
      !that.runExecutionData?.resultData
    ) {
        ...
        if (name[0] === "&") {
          ...
          if (!that.siblingParameters.hasOwnProperty(key)) {
            throw new ApplicationError(
              "Could not find sibling parameter on node",
              {
  ...
  private getNodeExecutionOrPinnedData({
    ...
    try {
      return this.getNodeExecutionData(
        nodeName,
        shortSyntax,
        branchIndex,
        runIndex
  ...
  private getNodeExecutionData(
    ...
    if (!shortSyntax) {
      ...
      const taskData =
        that.runExecutionData.resultData.runData[nodeName][runIndex].data!;
      ...
      if (outputIndex === undefined) {
        ...
        if (nodeConnection === undefined) {
          throw new ExpressionError(
            `connect "${that.contextNodeName}" to "${nodeName}"`,
            {
              runIndex: that.runIndex,
              itemIndex: that.itemIndex,
            }
    ...
    return new Proxy(
      {},
      {
        ...
        get(target, name, receiver) {
          ...
          const sourceData: ISourceData = that.executeData.source
            .main[0] as ISourceData;
  ...
  getDataProxy(opts?: {
    ...
    const createExpressionError = (
      ...
      if (
        isScriptingNode(that.activeNodeName, that.workflow) &&
        context?.functionOverrides
      ) {
        ...
        context.description =
          context.functionOverrides.description || context.description;
      ...
      if (context?.nodeCause) {
        ...
        if (
          ...
          (pinData || isScriptingNode(nodeName, that.workflow))
        ) {
          const moreInfoLink =
    ...
    function createBranchNotFoundError(
      node: string,
      item: number,
      cause?: string
    ) {
    ...
    function createPairedItemMultipleItemsFound(
      destNode: string,
      itemIndex: number
    ) {
    ...
    function pinDataToTask(
      pinData: INodeExecutionData[] | undefined
    ): ITaskData | undefined {
    ...
    const getPairedItem = (
      ...
      // Normalize inputs
      const [pairedItem, sourceData] = normalizeInputs(
        initialPairedItem,
        incomingSourceData
      );
      ...
      if (nextPairedItems.length === 0) {
        throw createMissingPairedItemError(
      ...
      const results = nextPairedItems.flatMap((nextPairedItem) => {
        ...
        const nextSource =
          nextPairedItem.sourceOverwrite ?? sourceArray[inputIndex];
      ...
      if (rest.some((r) => r !== first)) {
        throw createPairedItemMultipleItemsFound(
          destinationNodeName,
          pairedItem.item
        );
      }
    ...
    const handleFromAi = (
      ...
      if (!name || name === "") {
        throw new ExpressionError(
          "Add a key, e.g. $fromAI('placeholder_name')",
          {
            runIndex,
            itemIndex,
          }
      ...
      const inputData =
        that.runExecutionData?.resultData.runData[that.activeNodeName]?.[
          runIndex
        ].inputOverride;
    ...
    const base = {
      $: (nodeName: string) => {
        ...
        const ensureNodeExecutionData = () => {
          const runData = that?.runExecutionData?.resultData?.runData ?? {};

          if (
            !Object.prototype.hasOwnProperty.call(runData, nodeName) &&
            !getPinDataIfManualExecution(that.workflow, nodeName, that.mode)
          ) {
          ...
          {},
          {
            ...
            get(target, property, receiver) {
              ...
              if (
                ...
                if (activeNode) {
                  const parentMainInputNode =
                    that.workflow.getParentMainInputNode(activeNode);
                ...
                const pairedItemMethod = (itemIndex?: number) => {
                  if (itemIndex === undefined) {
                    if (property === PAIRED_ITEM_METHOD.ITEM_MATCHING) {
                      throw createExpressionError(
                        "Missing item index for .itemMatching()",
                        {
                          itemIndex,
                        }
                      );
                  ...
                  if (!input) {
                    throw createExpressionError(
                      ...
                      {
                        messageTemplate:
                          "Can’t get data for expression under ‘%%PARAMETER%%’ field",
                        functionality: "pairedItem",
                        functionOverrides: {
              ...
              if (property === "first") {
                ...
                return (branchIndex?: number, runIndex?: number) => {
                  branchIndex =
                    ...
                    that.workflow.getNodeConnectionIndexes(
                      that.activeNodeName,
                      nodeName
                    )?.sourceIndex ??
                    0;
        ...
        get(target, property, receiver) {
          ...
          if (property === "first") {
            return (...args: unknown[]) => {
              if (args.length) {
                throw createExpressionError(
                  "$input.first() should have no arguments"
                );
              }
          ...
          if (property === "last") {
            return (...args: unknown[]) => {
              if (args.length) {
                throw createExpressionError(
                  "$input.last() should have no arguments"
                );
              }
      ...
      $items: (nodeName?: string, outputIndex?: number, runIndex?: number) => {
        ...
        return that.getNodeExecutionData(
          nodeName,
          false,
          outputIndex,
          runIndex
    ...
    const throwOnMissingExecutionData =
      opts?.throwOnMissingExecutionData ?? true;
    ...
    return new Proxy(base, {
      ...
      get(target, name, receiver) {
        ...
        if (["$data", "$json"].includes(name as string)) {
          return that.nodeDataGetter(
            that.contextNodeName,
            true,
            throwOnMissingExecutionData
          )?.json;
        }
        if (name === "$binary") {
          return that.nodeDataGetter(
